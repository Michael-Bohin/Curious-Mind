class View {
    ...
    constructor() {
        ...
        if(el !== null) el.addEventListener("mousedown", ev => { window.location.href = "../portal.html"; });
        for(let i: number = 0; i < this.varianty.length; ++i) {
            let a: string[] = this.varianty[i]; 
            if(document.getElementById(a[1]) !== null) document.getElementById(a[1]).addEventListener("mousedown", ev => {
                this.nazev_hry = a[0]; 
    ...
    display_drag_count() {
        ...
        var seek: HTMLElement = document.querySelector('.seekbar');
        var fill: HTMLElement = document.querySelector('.fill');
        var handle = document.querySelector('.handle'); /* !!!!!! */
        var msg: HTMLElement = document.getElementById('pocet');

        document.getElementById('hrajem').addEventListener( 'mousedown', function(e) { view.display_game(parseInt(msg.innerHTML)); });
        function clamp(min: number,val: number,max: number){ return Math.min(Math.max(min,val),max); }
        function pocet(val: number):number { return 5 + Math.floor(val * 15); }
    ...
    display_game(c: number): void  { 
        ...
        document.addEventListener("keydown", ev => { if(ev.key == "Enter") this.post_answer(); });
    ...
    post_answer(): void {
        var inputValue = (<HTMLInputElement>document.getElementById("odpoved")).value;
        (<HTMLInputElement>document.getElementById("odpoved")).value = ""; // reset to empty string 
        console.log(`user's current answer is: ${inputValue}`);
        let pr: HTMLElement = document.getElementById("priklad");
        this.pocet_odpovedi += 1;
        console.log(`pocet odpovedi: ${this.pocet_odpovedi}`);
        let odp: HTMLElement = document.getElementById(`odp${this.pocet_odpovedi}`);
    ...
    add_ice_btn(input: string[][]): void {
        let temp_body: string = document.body.innerHTML;
    ...
    mistakes_move() {
        ...
        const init_left: number = 330;
        const spacing: number = 200;
        let counter: number = 0;
        let g: Game = game;
        for(let i: number = 1; i <= (game.total_incorrect + game.exercise_count); ++i) {
            if(counter == 5) {
                break; // move at most 5 mistakes, due to space reasons
            }
            let el: HTMLElement = document.getElementById(`odp${i}`);
            let text: string = el.innerHTML;
            let qq: string = text.split("=")[0];
            let aa: string = text.split("=")[1];
            let q: number = eval(qq);
            let a: number = parseInt(aa);
